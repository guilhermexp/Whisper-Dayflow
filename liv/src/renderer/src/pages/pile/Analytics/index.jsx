          ...
          <div className={styles.mainContent}>
            <Tabs.Root value={mainTab} onValueChange={setMainTab}>
              <Tabs.List className={styles.TabsList}>
                <Tabs.Trigger value="analytics" className={styles.TabTrigger}>
              ...
              <Tabs.Content value="analytics">
                <div className={styles.Container}>
                    ...
                    <div className={styles.SectionHeader}>
                      <div>
                        <div className={styles.SectionTitle}>
                          {t("analytics.recordingHistory")}
                        </div>
                        <div className={styles.SectionSubLabel}>
                          {t("analytics.recordingsStoredLocally", {
                      ...
                      <div className={styles.Grid2}>
                        <div style={{ padding: "8px", textAlign: "center" }}>
                          <div
                            ...
                            style={{ fontSize: "16px" }}
                          >
                            {analytics.productivity.wordsPerMinute.average?.toFixed(
                              0,
                            ) || "--"}
                          </div>
                          <div className={styles.StatLabel}>
                        ...
                        <div style={{ padding: "8px", textAlign: "center" }}>
                          ...
                          >
                            {formatProcessing(
                              analytics.productivity.processingTimeMs.average,
                            )}
                          </div>
                          <div className={styles.StatLabel}>
                  ...
                  {(!analytics ||
                    ((!analytics.sttModelRanking ||
                      analytics.sttModelRanking.length === 0) &&
                      (!analytics.enhancementModelRanking ||
              ...
              <Tabs.Content value="history">
                <div className={styles.HistoryList}>
                  {(() => {
                    console.log(
                      "[Analytics] History tab rendering, historyData length:",
                      historyData.length,
                    )
                    if (historyData.length > 0) {
                      console.log(
                        "[Analytics] First item keys:",
                        Object.keys(historyData[0]),
                      )
                      console.log(
                        "[Analytics] First item contextScreenshotPath:",
                        historyData[0].contextScreenshotPath,
                      )
                    }
                    return null
                  })()}
                  {historyData.length === 0 ? (
                    ...
                    historyData
                      ...
                      .map((item, index) => {
                        // Debug: log items with screenshots
                        if (item.contextScreenshotPath) {
                          console.log(
                            "[Analytics] Item with screenshot:",
                            item.id,
                        ...
                        if (screenshotSrc) {
                          console.log(
                            "[Analytics] Screenshot URL constructed:",
                            screenshotSrc,
                          )
                        }
                        ...
                        return (
                          ...
                          >
                            ...
                            {screenshotSrc && (
                              <div className={styles.HistoryScreenshot}>
                                <img
                                  src={screenshotSrc}
                                  alt="Screen context"
                                  className={styles.screenshotThumbnail}
                                  onClick={() => {
                                    // Open full-size screenshot in new window
                                    window.open(screenshotSrc, "_blank")
                                  }}
                                  onError={(e) => {
                                    console.error(
                                      "[Analytics] Image load error for:",
                                      item.id,
                                      "URL:",
                                      screenshotSrc,
                                    )
                                    // Fallback to legacy screenshot endpoint if direct path fails
                                    const target = e.currentTarget
                                    if (!target.dataset.fallbackUsed) {
                                      target.dataset.fallbackUsed = "1"
                                      const fallbackUrl = `assets://screenshot/${item.id}`
                                      console.log(
                                        "[Analytics] Trying fallback URL:",
                                        fallbackUrl,
                                      )
                                  ...
                                  onLoad={() => {
                                    console.log(
                                      "[Analytics] Image loaded successfully for:",
                                      item.id,
                                    )
                                  }}
                                />
                                <div className={styles.screenshotInfo}>
                                  {item.contextScreenAppName &&
                                    item.contextScreenAppName !== "Unknown" && (
                                      <div className={styles.screenshotApp}>
                                        {item.contextScreenAppName}
                                      </div>
                                    )}
                                  {item.contextScreenWindowTitle && (
                            ...
                            <div className={styles.HistoryMeta}>
                              ...
                              {(item.confidenceScore != null ||
                                ...
                                <span className={styles.MetaItem}>
                                  ...
                                  {formatConfidence(
                                    item.confidenceScore ?? item.accuracyScore,
                                  )}
                                </span>
                              )}
                              {item.fileSize && (
